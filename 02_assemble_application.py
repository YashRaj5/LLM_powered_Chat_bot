...
# MAGIC %md The purpose of this notebook is to define and persist the model to be used by the QA Bot accelerator.
...
# DBTITLE 1,Installing Required Libararies
# MAGIC %pip install langchain==0.0.166 tiktoken==0.4.0 openai==0.27.6 faiss-cpu==1.7.4 typing-inspect==0.8.0 typing_extensions==4.5.0 sentence-transformers==2.2.2
...
# DBTITLE 1,Import Required Libaraies
import re
import time
import pandas as pd
import mlflow
...
# MAGIC %md
# MAGIC # Explore Answer Generation
...
# MAGIC **NOTE:** The OpenAI API key used by the OpenAIEmbeddings object is speicified in an envrionment variable set during the earlier `%run` call to get configuration variables.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Retrieve Relevant Documetns

# COMMAND ----------

# DBTITLE 1,using OpenAI
...
# DBTITLE 1,using Hugginface
# open vector store to access embeddings
embeddings = HuggingFaceEmbeddings(model_name=config['hf_embedding_model_loc'])
vector_store = FAISS.load_local(embeddings=embeddings, folder_path=config['vector_store_path'])
...
# MAGIC %md
# MAGIC We can now turn our attention to the prompt that we will send to the model. This prompt needs to include placeholders for the question the user will submit and the document that we believe will provide the context for answering it.
# MAGIC
# MAGIC Please note that the prompt consists of multiple prompt elements, defined using prompt templates. In a nutshell, [prompt templates](https://python.langchain.com/en/latest/modules/prompts/chat_prompt_template.html) allow us to define the basic structure of a prompt and more easily substitute variable data into them to trigger a response. The system message prompt shown here provides instruction to the model about how we want it to respond. The human message template provides the details about the user-initiated request.
# MAGIC
...
# DBTITLE 1,Define Chain to Generate Response
...
# MAGIC %md
# MAGIC To actually trigger a response, we will loop through each of our docs from highest to lowest relevance and attempt to elicit a response. Once we get a valid response, we'll stop.
# MAGIC
...
# MAGIC %md
# MAGIC # Assemble Model for Deployment
...
# MAGIC
# MAGIC To create a custom MLflow model, all we need to do is define a class wrapper of type mlflow.pyfunc.PythonModel. The init method will initialize an instance of our QABot class and persist it to an class variable. And a predict method will serve as the standard interface for generating a response. That method will receive our inputs as a pandas dataframe but we can write the logic with the knowledge that it will only be receiving one user-provided question at a time:
...
# MAGIC %md
# MAGIC We can then instantiate our model and log it to the [MLflow registry](https://docs.databricks.com/mlflow/model-registry.html) as follows:
...
with mlflow.start_run():
  _ = (
    mlflow.pyfunc.log_model(
      python_model=model,
      extra_pip_requirements=['langchain==0.0.166', 'tiktoken==0.4.0', 'openai==0.27.6', 'faiss-cpu==1.7.4', 'typing-inspect==0.8.0', 'typing_extensions==4.5.0', 'sentence-transformers==2.2.2'],
      artifact_path='model',
